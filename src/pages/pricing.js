...
const PricingPage = () => {
  ...
  const getUsageInfo = async () => {
    const stats = await axios.get(
      `https://web-production-11db1.up.railway.app/api/usage_stats/`
    )
    if (stats.data.pro_count > 3) {
  ...
  return (
    <Layout>
      ...
      <Section>
        ...
        <Text>
          We don't run advertisements, sell your data, or require a subscription
          but we do need to pay the bills. Here is a progress bar for the
          minimum amount of money we need to make each month to keep the app
          running.
        </Text>
        <div
          style={{
        ...
        <ProgressBar>
          <div style={{ flex: revenue }} />
          <div style={{ flex: 300 - revenue }} />
        </ProgressBar>
      </Section>
    </Layout>
  )
